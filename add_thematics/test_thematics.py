...
def setup_theme_page(browser):
    ...
    """ Проверка формы добавления тематики"""
    ...
    wait_present(browser, 'XPATH', thematics.geogr_add_btn, "Кнопка'Добавить' для добавления страны и региона отсутствует")
...
def test_input_check(browser):
    ...
    """ Название """
    browser.find_element(By.XPATH, thematics.name).send_keys("Футбол")
    ...
    assert browser.find_element(By.XPATH, thematics.event_class).get_attribute('value') == "В мире"  # проверка заполнения поля
    ...
    wait_text_present(browser, 'XPATH', thematics.chan_created, 'https://t.me/rflive', 'Текст отсутствует в поле отображения добавленного канала')
    ...
    assert browser.find_element(By.XPATH, thematics.use_ai).get_attribute('value') == "on"

    """ Ключевые фразы и слова """
    browser.find_element(By.XPATH, thematics.key_ph).send_keys("Футбол")
    ...
    wait_text_present(browser, 'XPATH', thematics.key_ph_created, 'Футбол', 'Текст не появился в элементе')
